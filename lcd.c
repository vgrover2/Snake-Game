...
void HAL_LCD_SpiInit(void)
{
    ...
    EUSCI_B0->CTLW0 =   EUSCI_B_CTLW0_CKPH |            // Phase of 1
                        ...
                        EUSCI_B_CTLW0_STEM |            // UCxSTE digital output (not necessary for us)
...
void HAL_LCD_writeCommand(uint8_t command)
{
    ...
    while((EUSCI_B0_SPI->STATW & UCBUSY) == UCBUSY){}

    // Transmit data
    EUSCI_B0_SPI->TXBUF = command;
...
/*******************************************************************************
* Function Name: lcd_draw_line_horizontal
********************************************************************************
...
void lcd_draw_line_horizontal(
  ...
  uint16_t fColor
    ...
    // Set the height and widths in pixels
    if ((x + len) >= 131){
        width_pixels = 131 - x;
    } else {
        width_pixels = len;
    ...
    // Set draw frame
    Crystalfontz128x128_SetDrawFrame(x, y, x + width_pixels, y);
...
void lcd_draw_line_vertical(
  ...
  uint16_t fColor
    ...
    // Set draw frame
    Crystalfontz128x128_SetDrawFrame(x, y, x, y + height_pixels);
...
void lcd_draw_pixel(
  ...
  uint16_t fColor
    ...
    uint16_t i, j;
    uint16_t height_pixels = 1;
    uint16_t width_pixels = 1;

    // Set draw frame
    Crystalfontz128x128_SetDrawFrame(x, y, x, y);
